...
@it.rally_in_mem
def test_run_with_direct_internet_connection(shared_setup, cfg, http_proxy, fresh_log_file):
    assert it.esrally(cfg, "list tracks") == 0
    assert it.find_log_line(fresh_log_file, "Connecting directly to the Internet")


@it.rally_in_mem
def test_anonymous_proxy_no_connection(shared_setup, cfg, http_proxy):
    env = dict(os.environ)
...
@it.rally_in_mem
def test_authenticated_proxy_user_can_connect(shared_setup, cfg, http_proxy):
    env = dict(os.environ)
