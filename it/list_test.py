...
def test_list_races(shared_setup, cfg):
    assert it.esrally(cfg, "list races") == 0


@it.rally_in_mem
def test_list_cars(shared_setup, cfg):
    assert it.esrally(cfg, "list cars") == 0


@it.rally_in_mem
def test_list_elasticsearch_plugins(shared_setup, cfg):
    assert it.esrally(cfg, "list elasticsearch-plugins") == 0


@it.rally_in_mem
def test_list_tracks(shared_setup, cfg):
...
@it.rally_in_mem
def test_list_telemetry(shared_setup, cfg):
