...
def create_arg_parser():
    ...
    for p in [start_parser, stop_parser]:
        p.add_argument("--trial-id",
                       help="provisioned trial ID of node to manage",
                       default=None)
...
def main():
    ...
    if sub_command == "provision":
        ...
        if args.node_ips:
            cfg.add(config.Scope.applicationOverride, "provisioning", "node.ips", opts.csv_to_list(args.node_ips))
        if args.docker:
            cfg.add(config.Scope.applicationOverride, "provisioning", "docker", args.docker)
    if sub_command == "start" or sub_command == "stop":
        if args.trial_id:
            cfg.add(config.Scope.applicationOverride, "system", "trial.id", args.trial_id)
