...
readonly DOCKER_IMAGE="${DOCKER_IMAGE:-elastic/rally}"
...
function test_docker_release_image {
    ...
    if [[ "${DEVELOPMENT}" == "YES" ]]; then
        ...
        actual_version=$(docker run --rm ${DOCKER_IMAGE}:${RALLY_VERSION_TAG} esrally --version | cut -d ' ' -f 2,2)
    fi
    ...
    actual_version=$(docker inspect --format '{{ index .Config.Labels "org.label-schema.version"}}' ${DOCKER_IMAGE}:${RALLY_VERSION_TAG})
    if [[ ${actual_version} != ${RALLY_VERSION} ]]; then
    ...
    actual_license=$(docker inspect --format '{{ index .Config.Labels "license"}}' ${DOCKER_IMAGE}:${RALLY_VERSION_TAG})
