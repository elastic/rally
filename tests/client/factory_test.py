...
class TestEsClientFactory:
    ...
    @mock.patch("esrally.client.asynchronous.RallyAsyncElasticsearch")
    def test_http_compress_false(self, mocked_es):
        hosts = [{"host": "localhost", "port": 9200}]
        client_options = {
            "use_ssl": True,
            "verify_certs": True,
        ...
        assert f.create_async()

        mocked_es.assert_called_once_with(
            distribution_version=None,
            distribution_flavor=None,
            hosts=["https://localhost:9200"],
            transport_class=RallyAsyncTransport,
    ...
    @mock.patch("esrally.client.asynchronous.RallyAsyncElasticsearch")
    def test_http_compress_true(self, mocked_es):
        hosts = [{"host": "localhost", "port": 9200}]
        client_options = {
            "use_ssl": True,
            "verify_certs": True,
    ...
    @mock.patch("esrally.client.asynchronous.RallyAsyncElasticsearch")
    def test_http_compress_not_set(self, mocked_es):
        hosts = [{"host": "localhost", "port": 9200}]
        client_options = {
            "use_ssl": True,
            "verify_certs": True,
