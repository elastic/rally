...
class DummyClient(Client):
    ...
    def get(self, url: str, stream: Writable, ranges: RangeSet = NO_RANGE) -> Head:
        ...
        if data:
            stream.write(data)
        return Head.create(url, ranges=ranges, content_length=len(data))
...
@cases(
    ...
    default=TransferCase(want_done="0-1023"),
...
def test_transfer(case: TransferCase, executor: DummyExecutor, tmpdir: os.PathLike) -> None:
    ...
    head = client.head(case.url)
    path = os.path.join(str(tmpdir), os.path.basename(head.url))
