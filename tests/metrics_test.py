...
class TransportErrors:
    ...
    def side_effects(self):
        ...
        for rnd_code in self.rnd_err_codes:
            exc = elasticsearch.exceptions.TransportError(rnd_code, TransportErrors.err_return_codes[rnd_code])
            exc.status = rnd_code
            side_effect_list.append(exc)
...
class TestEsClient:
    ...
    def test_fails_after_too_many_errors(self, mocked_sleep):
        def random_transport_error(rnd_resp_code):
            exc = elasticsearch.exceptions.TransportError(rnd_resp_code, TransportErrors.err_return_codes[rnd_resp_code])
            exc.status = rnd_resp_code
            raise exc
