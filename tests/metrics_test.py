...
class TestEsMetrics:
    ...
    class OpenCase:
        ...
        config_overwrite_existing_templates: str | None = None
        want_put_template: bool = False

    @cases.cases(
        OpenCase("no-create", create=False),
        OpenCase("default", want_put_template=True),
        OpenCase("template-exists", template=rally_metric_template()),
        OpenCase(
            "config_overwrite_existing_templates_true",
            template=rally_metric_template(),
            config_overwrite_existing_templates="true",
            want_put_template=True,
        ),
        OpenCase(
            "config_overwrite_existing_templates_false",
            template=rally_metric_template(),
            config_overwrite_existing_templates="false",
        ),
    )
    def test_open(self, case):
        ...
        if case.config_overwrite_existing_templates is not None:
            self.cfg.add(
                scope=config.Scope.application,
                section="reporting",
                key="datastore.overwrite_existing_templates",
                value=case.config_overwrite_existing_templates,
            )
