...
class TestSearchableSnapshotsOverride:
    params = {
        "repository": "my-repository",
        "snapshot": "my-snapshot",
        "master_timeout": 1,
        "body": {"index": "my-index", "renamed_index": "my-renamed-index", "ignore_index_settings": True},
        "storage": "full_copy",
        "wait_for_completion": True,
    }

    @mock.patch("esrally.client.asynchronous.SearchableSnapshotsClient")
    @pytest.mark.asyncio
    async def test_RallySearchableSnapshotsClient_rewrites_kwargs(self, es_searchable_snapshots):
        es = RallyAsyncElasticsearch(hosts=["http://localhost:9200"])
        es_searchable_snapshots.mount = mock.AsyncMock(return_value={})
        # simulating a custom runner that hasn't been refactored
        # to suit the new 'elasticsearch-py' 8.x kwarg only method signature
        await es.searchable_snapshots.mount(
            ...
            wait_for_completion=self.params["wait_for_completion"],
        )

        es_searchable_snapshots.mount.assert_awaited_once_with(
            es.searchable_snapshots,
            repository=self.params["repository"],
            snapshot=self.params["snapshot"],
            index=self.params["body"]["index"],
            renamed_index=self.params["body"]["renamed_index"],
            ignore_index_settings=self.params["body"]["ignore_index_settings"],
            storage=self.params["storage"],
            wait_for_completion=self.params["wait_for_completion"],
        )
    ...
    @mock.patch("esrally.client.asynchronous.SearchableSnapshotsClient")
    @pytest.mark.asyncio
    async def test_RallySearchableSnapshotsClient_rewrites_args(self, es_searchable_snapshots):
        es = RallyAsyncElasticsearch(hosts=["http://localhost:9200"])
        es_searchable_snapshots.mount = mock.AsyncMock(return_value={})
        # simulating a custom runner that hasn't been refactored
        # to suit the new 'elasticsearch-py' 8.x kwarg only method signature
        await es.searchable_snapshots.mount(
            ...
            self.params["body"],
            self.params["master_timeout"],
            self.params["storage"],
            self.params["wait_for_completion"],
        )
        ...
        es_searchable_snapshots.mount.assert_awaited_once_with(
            ...
            storage=self.params["storage"],
            wait_for_completion=self.params["wait_for_completion"],
            master_timeout=self.params["master_timeout"],
        )

    @mock.patch("esrally.client.asynchronous.SearchableSnapshotsClient")
    @pytest.mark.asyncio
    async def test_RallySearchableSnapshotsClient_rewrites_missing_args(self, es_searchable_snapshots):
        es = RallyAsyncElasticsearch(hosts=["http://localhost:9200"])
        es_searchable_snapshots.mount = mock.AsyncMock(return_value={})
        # simulating a custom runner that hasn't been refactored
        # to suit the new 'elasticsearch-py' 8.x kwarg only method signature
