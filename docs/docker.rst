...
As a quality of life improvement, the ``rally.ini`` and ``logging.json`` files can use ``${RALLY_CONFIG_DIR}`` in ``rally.ini`` and ``${LOG_PATH}`` in ``logging.json`` in order to
make the files more portable. For example:

* In ``rally.ini``, you can set ``root.dir = ${RALLY_CONFIG_DIR}/benchmarks`` instead of hard-coding the path as ``/rally/.rally/benchmarks``
