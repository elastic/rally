...
* ``datastore.use_data_streams`` (default: ``true``): If ``true``, Rally writes to ``rally-metrics-v1``, ``rally-races-v1`` and ``rally-results-v1`` data streams and if ``false`` to date based ``rally-metrics-YYYY-MM``, ``rally-races-YYYY-MM`` and ``rally-results-YYYY-MM`` indices. When ``true`` Rally creates a default ILM policy with ``50 GB`` rollover and a composable index template for each stream, which is composed of mappings, a default ILM policy and an empty @custom component which can be used to apply custom settings.
...
* ``datastore.overwrite_existing_templates`` (default: ``false``): Existing Rally index teplates are replaced only when this option is ``true``. In the case of ``datastore.use_data_streams`` is ``true``, this option applies to both the default composable index template and the default ILM policy.
