...
* ``datastore.overwrite_existing_templates`` (default: ``false``): Existing Rally index templates are replaced only when this option is ``true``. When ``datastore.use_data_streams`` is ``true``, this option applies to component templates, the composable index template and the ILM policy. The index template for the ``rally-annotations`` index is also only replaced when this option is ``true``.
