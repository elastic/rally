...
* ``datastore.overwrite_existing_templates`` (default: ``false``): Existing Rally index templates are replaced only when this option is ``true``. When ``datastore.use_data_streams`` is ``true``, this option applies to both the default composable index template and the default ILM policy.
