...
* ``jfr-recording-template``: The name of a custom flight recording template. It is up to you to correctly install these recording templates on each target machine. If none is specified, the default recording template of Java flight recorder is used. The previous name of this parameter, ``recording-template``, is deprecated but still accepted.
